...
- `scripts/benchmark.py` — runs both Python algos N=30 (`--runs/--iters/--agents/--p-head/--step/--pc/--pm/--noise/--seed-base` flags) → `exports/bench/{meta,ga}_avg.csv` (`iter,avg_best,median_best,std_best,avg_mean`); `scripts/plot_compare.py` → `exports/bench/compare.png` (3-line semilogy).
- `examples/bench_gsa.cpp` — same N-run protocol in C++ (`bench` target, `GSA_BUILD_BENCH`) → `exports/bench/gsa_avg.csv` (same 5 columns; median via two `nth_element` calls like `ComputeFitnessStats`). Fairness contract: same dims/bounds/objective/pop/iters, distinct deterministic seeds, minimize everywhere.
