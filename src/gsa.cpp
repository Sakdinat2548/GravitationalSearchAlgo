...
void GravitationalSearchAlgorithm::ComputeAccelerations(
  ...
  auto comp = [minimizing = config_.minimize, &f = s.fitness](int a, int b) {
  ...
  for (int i : std::views::iota(0, config_.n_agents)) {
    ...
    for (int k : std::views::iota(0, k_best_count)) {
...
void GravitationalSearchAlgorithm::ValidateInputs(
  ...
  for (size_t i : std::views::iota(size_t{0}, lower.size())) {
    if (lower[i] > upper[i]) {
      throw std::invalid_argument(
...
GsaResult GravitationalSearchAlgorithm::Optimize() const {
  ...
  for (int k : std::views::iota(1, config_.max_iter + 1)) {
